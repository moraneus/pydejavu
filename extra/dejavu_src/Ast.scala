  ...
  def translate(executionMode: Boolean, prePropertySynthesisCode: String): Unit = {
    ...
    writeln(
      s"""
         ...
         |      if (parentDir.mkdirs()) {
         |        println(s" ***Created the parent directory: $${parentDir.getAbsolutePath}")
         |      } else {
         |        println(s" ***Failed to create the parent directory: $${parentDir.getAbsolutePath}")
         |        return false
         |      }
         |    }
         |
         |   println("Bits: " + bitsValue)
         |   println("Mode: " + modeValue)
         |   println("PrintStat: " + printStatValue)
         ...
         |    case _ =>
         |      moni_.lineNr += 1
         |      val resultMap: Map[String, Boolean] = moni_.submit(name, args)
